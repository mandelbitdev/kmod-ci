...
run_rhel_builder_container() {
	...
	docker run --rm --privileged \
		...
		-e RHEL_10_BETA_X86_64_PRODUCT_CERT \
		-e RHEL_ORG_ID \
...
cleanup_rhel_registration() {
	...
	rm -rf \
		...
		"${rootfs}/etc/pki/product-default/486.pem" \
		"${rootfs}/etc/rhsm" \
		"${rootfs}/etc/yum.repos.d/redhat.repo" \
		"${rootfs}/var/lib/rhsm"

	rm -f /etc/pki/product-default/486.pem
...
build_rhel() {
	...
	baseos_repo="rhel-${releasever}-for-${rhel_repo_arch}-baseos-rpms"
	appstream_repo="rhel-${releasever}-for-${rhel_repo_arch}-appstream-rpms"
	crb_repo="codeready-builder-for-rhel-${releasever}-${rhel_repo_arch}-rpms"

	if [ "${variant}" = beta ]; then
		if [ "${releasever}" != 10 ] || [ "${rhel_repo_arch}" != x86_64 ]; then
			echo "Unsupported RHEL beta target: ${releasever} ${rhel_repo_arch}" >&2
			exit 1
		fi

		require_rhel_secret RHEL_10_BETA_X86_64_PRODUCT_CERT

		# Beta repositories are visible only after installing the beta
		# product certificate, mirroring what a beta installation ISO
		# provides before registration.
		printf '%s\n' "${RHEL_10_BETA_X86_64_PRODUCT_CERT}" \
			> /etc/pki/product-default/486.pem

		baseos_repo="rhel-10-for-x86_64-baseos-beta-rpms"
		appstream_repo="rhel-10-for-x86_64-appstream-beta-rpms"
		crb_repo="codeready-builder-beta-for-rhel-10-x86_64-rpms"

		# The activation key may also expose GA repos; keep this rootfs
		# pinned to beta so the target actually tracks the pre-release
		# ABI.
		repo_args+=(--disable="*")
	fi
	...
	repo_args+=(
		--enable="${baseos_repo}"
		--enable="${appstream_repo}"
		--enable="${crb_repo}"
	)
	if ! repo_output=$(subscription-manager repos "${repo_args[@]}" 2>&1); then
		printf '%s\n' "${repo_output}" >&2
		exit 1
	fi
...
build_opensuse_tumbleweed() {
	local repo_dir="${script_dir}/../repos/opensuse-tumbleweed-${rpm_arch}"
